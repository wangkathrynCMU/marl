...
class MultiAgentPolicyModel(nn.Module):
    def __init__(self, dynamics_model, agents, s_dim, a_dim, d_model, h_dim):
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=64, dim_feedforward = 512, nhead=8, batch_first=True)
    ...
    def sample_next_action(self, state):
        action_mean, action_stdev = self.forward(state)
        next_action = torch.normal(action_mean, action_stdev)
        return next_action
    
    def forward_to_dictionary(self, state):
        # i think this only works if only one state is passed in
        action = self.sample_next_action(state).cpu().numpy()
        output = dict()
        index = 0
        for agent in agents:
...
def train(model, dynamics_model, start_states, opt, exploration, ep_len, epochs, prev_epochs, device):
    ...
    losses = []
    # do i want this outer epoch loop?
    for epoch in range(epochs):
        ...
        for i in range(ep_len):
            actions = model.sample_next_action(states)
