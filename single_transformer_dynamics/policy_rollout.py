...
def save_frames_as_gif(frames, filename='output.gif', duration=0.1):
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for i in range(ep_len):
            ...
            # rel_pos = np.concatenate((predictedx, predicted_state[:,3].numpy().reshape(-1, 1)), axis = 1)
            # buffer.append(rel_pos)
        

            # yrel = np.concatenate((predictedy, predicted_state[:,4].numpy().reshape(-1, 1)), axis = 1)
    save_frames_as_gif(frames, filename='policy_rollout.gif')
