...
class MADynamicsModel(nn.Module):
    def __init__(self, s_dim, a_dim, d_model, h_dim):
        super(MADynamicsModel, self).__init__()
    ...
    def get_loss(self,states,actions,next_states, rewards):
        ...
        print("next state reward shape:", next_state_reward.shape())
    ...
    def sample_next_state_reward(self, states, actions):
        ...
        return state, next_state_mean[...,-1:]
    

def train(model, opt, N_train_epochs, device):
    losses = list()
...
def make_model_dir():
    ...
    folder = "..\\model_checkpoints\\online_multiagent_dynamics"
    run_dir = folder + "\\online_multiagent_dynamics_" + str_date_time + "_" + str(env_name) + '_wd_' +str(wd)+'_lr_'+str(lr)+'_hdim_' + str(h_dim)
    os.mkdir(run_dir)
    filepath = run_dir + '\\online_multiagent_dynamics_'
    return filepath
...
if __name__ == '__main__':
    ...
    if comet:
        ...
        project_name="online-multiagent-dynamics",
    ...
    for i in range(n_epochs):
        ...
        train_loss = train(model, model_optimizer, N_train_epochs, device = device)
        ...
        # ONLINE LEARNING
        new_states, new_actions, new_next_states, new_rewards = gather_data(onlineN, ep_len, env_name, env_config)
        
        # if torch.cuda.is_available():
        #     new_states = new_states.cuda()
        #     new_actions = new_actions.cuda()
        #     new_next_states = new_next_states.cuda()
        #     new_rewards = new_rewards.cuda()

        new_dataset = torch.utils.data.TensorDataset(new_states,new_actions,new_next_states, new_rewards)
        
        data_size = new_states.shape[0]
        N_train = int((1-test_split)*data_size)
        N_test = data_size - N_train

        new_train_data, new_test_data = torch.utils.data.random_split(new_dataset, [N_train, N_test])

        train_data = torch.utils.data.ConcatDataset([train_data, new_train_data])
        test_data = torch.utils.data.ConcatDataset([train_data, new_train_data])
        ...
        if test_loss < min_test_loss and comet:
